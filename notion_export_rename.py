...
def rename(path_obj):  # Rename file/dir name.
...
def rename_links_in_file(path):  # Rename links in *.md files.
    ...
    with open(path, "r+", encoding="UTF-8") as f:
        ...
        if matches:
            for el in matches:
                ...
                #  list_to_file.append(unquote_el + "\n")
                #  
                #  with open("test/test.txt", "w", encoding="UTF-8") as f:
                #      f.writelines(list_to_file)
...
def parse_folder(path):  # Getting files and subdirectories of a directory
    path_obj = Path(path)
...
def unzip_file(path):  # Unpacking a *.zip archive.
...
def main():
    ...
    try:
        ...
        print(f'{ex}\n\nSpecify the correct path to the archive in the "settings.json" file.\n')
        return
